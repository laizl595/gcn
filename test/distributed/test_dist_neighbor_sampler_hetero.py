...
def create_hetero_data(rank, world_size, temporal=False):
    ...
    data = HeteroData()
    data['paper'].node_id = torch.tensor([0, 1, 2, 8, 9], dtype=torch.int64)
    data['author'].node_id = torch.tensor([3, 4, 5, 6, 7], dtype=torch.int64)
...
def dist_neighbor_sampler_hetero(
    ...
    if rank == 0:  # Seed nodes:
        ...
        input_node = torch.tensor([5, 7], dtype=torch.int64) # todo change to nodes from different partitions
